...
class ProbabilisticMap {
 ...
 private:
  ...
  // voxels that have been hit/missed in the current scan iteration
  std::vector<CoordT> _miss_coords;
  std::vector<CoordT> _hit_coords;

  mutable Bonxai::VoxelGrid<CellT>::Accessor _accessor;

  // clears all voxels from the origin to the hit/miss point
...
inline void ProbabilisticMap::insertPointCloud(
    const std::vector<PointT, Alloc>& points, const PointT& origin,
    double max_range) {
